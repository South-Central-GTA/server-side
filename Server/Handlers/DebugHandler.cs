...
public class DebugHandler : ISingletonScript
{
    public DebugHandler()
    {
        AltAsync.OnClient<ServerPlayer, string, float, float, float, float, float, float>("data:sendcamerainfo", OnSendCameraInfo);
    }

    private void OnSendCameraInfo(ServerPlayer player, string name, float x, float y, float z, float roll, float pitch, float yaw)
    {
        if (!player.Exists)
        {
            return;
