...
public class OrderProductsHandler : ISingletonScript
{
    ...
    public OrderProductsHandler(IOptions<CompanyOptions> companyOptions, IOptions<DeliveryOptions> deliveryOptions,
        DeliveryService deliveryService, BankAccountService bankAccountService, GroupService groupService,
        GroupModule groupModule, PhoneModule phoneModule, DeliveryModule deliveryModule)
    {
    ...
    private async void OnOrderProducts(ServerPlayer player, int phoneId, int amount)
    {
        ...
        if (await _deliveryService.Exists(d =>
                d.OrderGroupModelId == group.Id && d.Status != DeliveryState.DELIVERD &&
