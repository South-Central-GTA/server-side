    ...
    : ITransientScript
{
    ...
    public async Task<bool> CanCarry(ServerPlayer player, ItemCatalogIds itemCatalogId, int amount = 1)
    {
        ...
        switch (result)
        {
            ...
            case CanCarryErrorType.LIMIT:
                ...
                player.SendNotification($"In einem Inventar können maximal {catalogItem.MaxLimit} Stück des Items '{catalogItem.Name}' liegen.", NotificationType.ERROR);
                return false;
    ...
    public async Task<bool> CanCarry(ServerPlayer player, int inventoryId, float weight)
    {
        ...
        if (!canCarry)
        {
            player.SendNotification("Das Inventar deines Charakters ist voll, die Items konnten nicht hinzugefügt werden.", NotificationType.ERROR);
        }
    ...
    public async Task OpenInventoryUiAsync(ServerPlayer player)
    {
        ...
        {
            player.SendNotification("Du kannst jetzt nicht in das Inventar deines Charakters schauen.", NotificationType.ERROR);
            return;
        }
    ...
    public async Task UpdateAmmo(ServerPlayer player, string ammoJson)
    {
        ...
        var pistolAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.AMMO_PISTOL);
        if (pistolAmmo != null)
        {
        ...
        var machineGunAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.AMMO_MACHINE_GUN);
        ...
        var assaultAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.AMMO_ASSAULT);
        if (assaultAmmo != null)
        {
        ...
        var sniperAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.AMMO_SNIPER);
        if (sniperAmmo != null)
        {
        ...
        var shotgunAmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.AMMO_SHOTGUN);
        if (shotgunAmo != null)
        {
        ...
        var lmgAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.AMMO_LIGHT_MACHINE_GUN);
        if (lmgAmmo != null)
        {
        ...
        var baseballAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.WEAPON_BASEBALL);
        ...
        var bzGasAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.WEAPON_BZ_GAS);
        if (bzGasAmmo != null)
        {
        ...
        var flareAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.WEAPON_FLARE);
        if (flareAmmo != null)
        {
        ...
        var grenadeAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.WEAPON_GRENADE);
        if (grenadeAmmo != null)
        {
        ...
        var molotovAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.WEAPON_MOLOTOV_COCKTAIL);
        if (molotovAmmo != null)
        {
        ...
        var snowballAmmo =
            player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => i.CatalogItemModelId == ItemCatalogIds.WEAPON_SNOWBALL);
    ...
    public async Task UpdateInventoryUIs(ServerPlayer player, InventoryModel inventoryModel)
    {
        ...
        {
            var players = Alt.GetAllPlayers().Where(p => p.Dimension == inventoryModel.HouseModelId && !p.Equals(player));
    ...
    private async Task<List<InventoryModel>> GetInventories(ServerPlayer player)
    {
        ...
        if (player.DefaultInventories.Count != 0)
        {
            inventories.AddRange(player.DefaultInventories);
        }
        ...
        if (player.Dimension != 0)
        {
            ...
            {
                var houseKeyItem = player.CharacterModel.InventoryModel.Items.FirstOrDefault(i => house.Keys.Any(k => k == i.Id));
        ...
        foreach (var group in await _groupService.Where(g => g.Members.Any(m => m.CharacterModelId == player.CharacterModel.Id)))
        {
