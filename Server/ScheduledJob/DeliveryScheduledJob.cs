...
public class DeliveryScheduledJob
    : ScheduledJob
{
    ...
    public override async Task Action()
    {
        ...
        acceptedDeliveries.FindAll(d => (DateTime.Now - d.LastUsage).TotalMinutes >= _deliveryOptions.PickupTime - 5).ForEach(d =>
        {
            ...
            player?.SendNotification("Deine Lieferung ist in 5 Minuten fehlgeschlagen da du sie nicht rechtzeitig am Hafen aufgeladen hast.", NotificationType.WARNING);
        });

        acceptedDeliveries.FindAll(d => (DateTime.Now - d.LastUsage).TotalMinutes >= _deliveryOptions.PickupTime).ForEach(d =>
        {
            if (!d.SupplierCharacterId.HasValue)
            {
                return;
            ...
            if (player != null)
            {
                player.SendNotification("Dein Charakter hat es nicht geschafft rechtzeitig die Lieferung am Hafen entgegenzunehmen.", NotificationType.ERROR);
        ...
        pausedDeliveries.FindAll(d => (DateTime.Now - d.LastUsage).TotalMinutes >= _deliveryOptions.ResetTime).ForEach(d =>
        {
            d = ClearProductDelivery(d);
        });
