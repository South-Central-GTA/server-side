...
internal class JailPlayerCommand : ISingletonScript
{
    ...
    [Command("jail", "Packe einen Charakter ins Gefängnis.", Permission.NONE, new[] { "Spieler ID", "Dauer in Minuten" })]
    public async void OnExecute(ServerPlayer player, string expectedPlayerId, string expectedDuration)
    {
        ...
        {
            player.SendNotification("Bitte gebe ein richtige Zahl für die Dauer in Minuten an.", NotificationType.ERROR);
            return;
        }
        ...
        player.SendNotification($"Dein Charakter hat {target.CharacterModel.Name} ins Gefängnis gesperrt.", NotificationType.SUCCESS);
    }
}